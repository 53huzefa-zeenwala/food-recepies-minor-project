...
import { fetchMealTypeData } from '../utils/FetchApi'
...
const MealType = () => {
    ...
    const getRecipes = async () => {
     console.log("mealtype", mealType);
      const data = await fetchMealTypeData(mealType ?? [], query);
      ...
      <div className='App'>
        ...
        {recipes != undefined && recipes != null
          ? recipes?.map((recipe, index) => (
              <Recipes
                key={index}
                labels={[...(recipe.recipe?.dishType ?? []), ...(recipe.recipe?.cuisineType ?? []), ...(recipe.recipe?.healthLabels ?? [])]}
